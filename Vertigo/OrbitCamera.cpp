...
void OrbitCamera::Reset(DirectX::XMFLOAT3 position)
{
...
void OrbitCamera::Reset(float radius, float phi, float theta)
{
    m_radius = 1;
...
void OrbitCamera::Update(float zoomFactor, float deltaPhi, float deltaTheta)
{
    Dolly(zoomFactor);
...
void OrbitCamera::Update(Mouse::ButtonStateTracker tracker)
{
    ...
    if (tracker.rightButton == ButtonState::PRESSED || tracker.middleButton == ButtonState::PRESSED)
    {
...
void OrbitCamera::SetProjection(float fovAngleY, float aspectRatio, float nearZ, float farZ)
{
...
void OrbitCamera::TrackLastCursorPosition(Mouse::State const & state) noexcept
{
    m_lastCursorPos.x = state.x;
    m_lastCursorPos.y = state.y;
}

void OrbitCamera::Dolly(float zoomFactor) noexcept
{
    m_radius *= zoomFactor;
}

void OrbitCamera::RotateLeft(float deltaTheta) noexcept
{
    m_theta += deltaTheta;
}

void OrbitCamera::RotateUp(float deltaPhi) noexcept
{
