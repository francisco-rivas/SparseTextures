    ...
    fx::gltf::Document const & doc, std::size_t meshIndex, D3DDeviceResources const * deviceResources)
{
    ...
    ID3D12GraphicsCommandList5 * commandList = deviceResources->GetCommandList();
...
void D3DMesh::Render(D3DRenderContext & renderContext, bool wireframe)
{
    ...
    for (auto & meshPart : m_meshParts) {
        ...
        ShaderOptions options = renderContext.OverrideShaderOptions == ShaderOptions::None ? meshPart.ShaderConfig : renderContext.OverrideShaderOptions;

        if (options != renderContext.CurrentShaderOptions) {
            if (wireframe) {
                renderContext.CommandList->SetPipelineState(renderContext.WireframePipelineStateMap[options | ShaderOptions::USE_WIREFRAME_PSO].Get());
            } else {
...
void D3DMesh::Render(D3DMSAARenderContext & renderContext)
{
    MeshConstantBuffer meshParameters{};
    meshParameters.WorldViewProj = DirectX::XMMatrixTranspose(DirectX::XMLoadFloat4x4(&m_worldMatrix) * renderContext.ViewProj);
    meshParameters.World = DirectX::XMMatrixTranspose(DirectX::XMLoadFloat4x4(&m_worldMatrix));
    ...
    for (auto & meshPart : m_meshParts)
    {
        ...
        if (options != renderContext.CurrentShaderOptions) {
            renderContext.CommandList->SetPipelineState(renderContext.MSAAPipelineStateMap[options | ShaderOptions::IS_MSAA].Get());
            renderContext.CurrentShaderOptions = options;
        }
