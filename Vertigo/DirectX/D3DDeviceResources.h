...
public:
    ...
    ID3D12GraphicsCommandList5 * GetCommandList() const noexcept
    {
    ...
    bool IsMSAASupportedOnDevice() const noexcept
    {
        return m_isMSAASupported;
    }
    ...
    ID3D12Resource* GetMSAADepthStencil()
    {
        return m_msaaDepthStencil.Get();
    }
    ...
    Intel::VALAR_DESCRIPTOR& GetVALARDescriptor()
    {
        return m_valarDescriptor;
    }

    ID3D12Resource* GetVALARBuffer()
    {
        return m_valarBuffer->GetCommittedResource();
    }

    
    ColorBuffer* m_valarColorBuffer;
    Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> m_rtvDescriptorHeap;
    Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> m_valarDescriptorHeap;
 private:
    void MoveToNextFrame();
    void GetAdapter(IDXGIAdapter1 ** ppAdapter);
    void InitializeMSAA(UINT backBufferWidth, UINT backBufferHeight);
    void InitializeVALAR(UINT backBufferWidth, UINT backBufferHeight);
    ...
    // VALAR API objects
    Intel::VALAR_DESCRIPTOR                          m_valarDescriptor;
    Intel::VALAR_HARDWARE_FEATURES                   m_valarHWFeatures;

    ColorBuffer*                                    m_valarBuffer;
public:
    UINT                m_msaaSampleCount = 4;
    bool                m_isMSAASupported = false;
    // clang-format on
};