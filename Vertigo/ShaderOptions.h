...
inline std::vector<std::string> GetShaderDefines(ShaderOptions options)
{
    ...
    if (IsSet(options, ShaderOptions::USE_WIREFRAME_PSO))
    {
        defines.emplace_back("USE_WIREFRAME_PSO");
    }
    if (IsSet(options, ShaderOptions::IS_MSAA))
    {
        defines.emplace_back("IS_MSAA");
    }
    return defines;
}